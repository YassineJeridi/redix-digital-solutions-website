...
const ServiceForm = ({ onSubmit, onClose, editData }) => {
    ...
    // Form data - uses the model field names (projectName, projectStatus, etc.)
    ...
    const handleRevenueChange = (field, value) => {
        ...
        setFormData(prev => ({
            ...prev,
            revenueDistribution: { ...prev.revenueDistribution, [field]: numValue }
        }));
    };
    ...
    // Calculate tool percentages based on purchasePrice
    const recalcToolPercentages = (toolsUsage) => {
        if (toolsUsage.length === 0) return toolsUsage;
        const totalPrice = toolsUsage.reduce((sum, tu) => {
            const tool = tools.find(t => t._id === tu.tool);
            return sum + (tool?.purchasePrice || 0);
        }, 0);
        if (totalPrice === 0) {
            // Fallback: equal distribution if all prices are 0
            const equalPct = Math.round((100 / toolsUsage.length) * 100) / 100;
            return toolsUsage.map(t => ({ ...t, percentage: equalPct }));
        }
        return toolsUsage.map(tu => {
            const tool = tools.find(t => t._id === tu.tool);
            const price = tool?.purchasePrice || 0;
            return { ...tu, percentage: Math.round((price / totalPrice) * 10000) / 100 };
        });
    };
    ...
    const toggleToolUsage = (toolId) => {
        setFormData(prev => {
            ...
            newToolsUsage = recalcToolPercentages(newToolsUsage);
    ...
    const handleTeamMemberToggle = (memberId) => {
        setFormData(prev => {
            ...
            return { ...prev, teamMembers: newTeamMembers, teamMemberShares: newShares };
        });
    };
    ...
    const validate = () => {
        const newErrors = {};
        if (!formData.projectName.trim()) newErrors.projectName = 'Service name is required';
        ...
        if (formData.serviceProvided === 'Development') {
            if (!formData.development.description.trim()) {
                newErrors.developmentDescription = 'Service description is required for Development';
            }
        }
        ...
        <div className={styles.overlay}>
            <div className={styles.modal}>
                ...
                <form onSubmit={handleSubmit} className={styles.form}>
                        ...
                        <section className={styles.section}>
                            ...
                            <div className={styles.row}>
                                <div className={styles.formGroup}>
                                    ...
                                    <input type="number" value={formData.marketing.videosCount}
                                        onChange={(e) => handleNestedChange('marketing', 'videosCount', parseInt(e.target.value) || 0)} min="0" />
                                </div>
                                <div className={styles.formGroup}>
                                    <label>Posts Count</label>
                                    <input type="number" value={formData.marketing.postsCount}
                                        onChange={(e) => handleNestedChange('marketing', 'postsCount', parseInt(e.target.value) || 0)} min="0" />
                                </div>
                                <div className={styles.formGroup}>
                                    <label>Shooting Sessions</label>
                                    <input type="number" value={formData.marketing.shootingSessionsCount}
                                        onChange={(e) => handleNestedChange('marketing', 'shootingSessionsCount', parseInt(e.target.value) || 0)} min="0" />
                                </div>
                            </div>
                        </section>
                    )}
                    ...
                    {formData.serviceProvided === 'Production' && (
                        <section className={styles.section}>
                            ...
                            <div className={styles.row}>
                                <div className={styles.formGroup}>
                                    <label>Videos Count</label>
                                    <input type="number" value={formData.production.videosCount}
                                        onChange={(e) => handleNestedChange('production', 'videosCount', parseInt(e.target.value) || 0)} min="0" />
                                </div>
                                <div className={styles.formGroup}>
                                    <label>Pictures Count</label>
                                    <input type="number" value={formData.production.picturesCount}
                                        onChange={(e) => handleNestedChange('production', 'picturesCount', parseInt(e.target.value) || 0)} min="0" />
                                </div>
                                <div className={styles.formGroup}>
                                    <label>Shooting Sessions</label>
                                    <input type="number" value={formData.production.shootingSessionsCount}
                                        onChange={(e) => handleNestedChange('production', 'shootingSessionsCount', parseInt(e.target.value) || 0)} min="0" />
                                </div>
                            </div>
                        </section>
                    )}
                        ...
                        <section className={styles.section}>
                            ...
                            <div className={styles.row}>
                                <div className={styles.formGroup}>
                                    ...
                                    <select value={formData.development.platform}
                                ...
                                <div className={styles.formGroup}>
                                    ...
                                    <select value={formData.development.typeComplexity}
                    ...
                    {showToolsUsage && (
                        <section className={styles.section}>
                            <h3 className={styles.sectionTitle}>Tools Usage (auto-calculated by purchase price)</h3>
                            {errors.toolsUsage && <div className={styles.errorBox}>{errors.toolsUsage}</div>}
                                    ...
                                    return (
                                        <div key={tool._id} className={`${styles.toolItem} ${usage ? styles.toolItemActive : ''}`}>
                                            <label className={styles.toolLabel}>
                                                <input type="checkbox" checked={!!usage} onChange={() => toggleToolUsage(tool._id)} />
                                                <span>{tool.name}</span>
                                                <span className={styles.toolPriceBadge}>{tool.purchasePrice?.toLocaleString()} TND</span>
                                            </label>
                                            {usage && (
                                                <span className={styles.toolPercentBadge}>{usage.percentage.toFixed(1)}%</span>
                                            )}
                                        </div>
                                    );
                                })}
                    ...
                    {/* Team Assignment */}
                    <section className={styles.section}>
                        <h3 className={styles.sectionTitle}>Team Assignment & Share Distribution (must total 100%)</h3>
                        {errors.teamMemberShares && <div className={styles.errorBox}>{errors.teamMemberShares}</div>}
                                ...
                                return (
                                    <div key={member._id} className={styles.teamMemberCard}>
                                        <label className={styles.teamMemberHeader}>
                                            <input type="checkbox" checked={isSelected} onChange={() => handleTeamMemberToggle(member._id)} />
                                        ...
                                        {isSelected && memberShare && (
                                            <div className={styles.memberShareInputs}>
                                                <div className={styles.shareInput}>
                                                    <label>Share %</label>
                                                    <input type="number" value={memberShare.percentage}
                                                        onChange={(e) => handleTeamMemberShareChange(member._id, e.target.value)}
                                                        min="0" max="100" step="0.01" placeholder="0" />
                                                </div>
                                                <div className={styles.shareAmount}>
                                                    <label>Amount</label>
                                                    <span className={styles.amountValue}>{memberShare.amount.toFixed(2)} TND</span>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                    ...
                    <div className={styles.footer}>
                        ...
                        <button type="submit" className={styles.submitBtn} disabled={loading}>
                            <MdSave /> {loading ? 'Saving...' : (editData ? 'Update Service' : 'Create Service')}
                        </button>
                    </div>
                </form>
            </div>
